...
export default function Earnings() {
  ...
  const load = async (nextPage = page) => {
    ...
    try {
      ...
      setError(err.response?.data?.detail || 'Could not load earnings.')
  ...
  return <div className="space-y-6">
    <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-sm font-semibold text-[#16843d]">Transactions</p><h1 className="mt-1 text-3xl font-black dark:text-white">Earnings</h1><p className="mt-1 text-sm text-slate-500">Record every payment with date, currency, category, and description.</p></div><button className="btn-primary" onClick={openNew} disabled={!platforms.length}><Plus size={18}/>Add earning</button></div>
