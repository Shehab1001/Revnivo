...
const emptyForm = { name: '', website: '', default_currency: 'USD', logo: null }
...
export default function Platforms() {
  ...
  const openEdit = (p) => { setEditing(p); setForm({ name:p.name, website:p.website || '', default_currency:p.default_currency || 'USD', logo:null }); setError(''); setModal(true) }

  const submit = async (e) => {
    e.preventDefault(); setSaving(true); setError('')
    const fd = new FormData(); fd.append('name', form.name); fd.append('website', form.website); fd.append('default_currency', form.default_currency); if (form.logo) fd.append('logo', form.logo)
    try {
  ...
  return <div className="space-y-6">
    <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-sm font-semibold text-[#16843d] dark:text-[#7bea9d]">Sources</p><h1 className="mt-1 text-3xl font-black tracking-tight text-slate-950 dark:text-white">Platforms</h1><p className="mt-1 text-sm text-slate-500">Add or remove every platform you earn from, including its logo.</p></div><button className="btn-primary" onClick={openNew}><Plus size={18}/>Add platform</button></div>
    {items.length ? <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">{items.map((p) => <div key={p.id} className="card p-5"><div className="flex items-start justify-between gap-4"><div className="flex min-w-0 items-center gap-3"><PlatformAvatar platform={p} size="lg"/><div className="min-w-0"><h3 className="truncate font-bold text-slate-900 dark:text-white">{p.name}</h3><p className="text-sm text-slate-500">Default: {p.default_currency}</p></div></div><div className="flex gap-1"><button onClick={()=>openEdit(p)} className="rounded-lg p-2 text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800"><Pencil size={16}/></button><button onClick={()=>remove(p)} className="rounded-lg p-2 text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-950/40"><Trash2 size={16}/></button></div></div>{p.website && <a className="mt-5 inline-flex items-center gap-1 text-sm font-semibold text-indigo-600 hover:text-indigo-500" href={p.website} target="_blank" rel="noreferrer">Visit website <ExternalLink size={14}/></a>}</div>)}</div> : <EmptyState title="No platforms yet" text="Add Upwork, Fiverr, YouTube, a client portal, or any other income source."/>}
