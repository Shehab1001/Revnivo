...
export default function SupportChat() {
  ...
  return <div className="space-y-6" onClick={() => contextMenu && setContextMenu(null)}>
    ...
    <Card className="grid h-[min(720px,calc(100vh-12rem))] min-h-140 overflow-hidden border-white/8 bg-[#101114] text-white shadow-none md:grid-cols-[280px_1fr]" radius="lg">
      <aside className="min-h-0 overflow-y-auto border-b border-white/8 md:border-b-0 md:border-r"><div className="border-b border-white/8 p-3"><Input aria-label="Search people" placeholder="Search people..." value={search} onValueChange={setSearch} startContent={<Search size={16}/>} classNames={{ inputWrapper: 'bg-[#0b0c0f] border-white/8', input: 'text-white placeholder:text-[#62656e]' }}/></div><div className="divide-y divide-white/6">{visibleContacts.map((contact) => <button key={contact.id} onClick={() => selectContact(contact)} className={`flex w-full items-center gap-3 p-3 text-left transition ${contact.unread_count > 0 ? 'chat-contact-new' : ''} ${active?.id === contact.id ? 'bg-[#1688ff]/12' : 'hover:bg-white/5'}`}><div className="relative"><Avatar user={contact}/><span className={`absolute bottom-0 right-0 h-3 w-3 rounded-full border-2 border-[#101114] ${contact.online ? 'bg-[#25d17f]' : 'bg-[#62656e]'}`}/></div><div className="min-w-0 flex-1"><div className="flex items-center justify-between gap-2"><div className="truncate text-sm font-semibold">{contact.name}</div>{contact.last_message_at && <time className="shrink-0 text-[10px] text-[#777a84]">{new Date(contact.last_message_at).toLocaleTimeString('en-EG', { hour: 'numeric', minute: '2-digit' })}</time>}</div><div className="flex items-center justify-between gap-2"><div className={`flex items-center gap-1 truncate text-xs ${contact.typing ? 'text-[#4aa3ff]' : 'text-[#858891]'}`}>{contact.typing ? <><span>typing</span><span className="typing-dots"><i/><i/><i/></span></> : contact.last_message || 'No messages yet'}</div>{contact.unread_count > 0 && <Chip size="sm" color="primary" variant="solid" className="h-5 min-w-5 px-1 text-[10px]">{contact.unread_count}</Chip>}</div></div></button>)}</div></aside>
